...
public class InjectionMetadata {
	...
	public void checkConfigMembers(RootBeanDefinition beanDefinition) {
		...
		for (InjectedElement element : this.injectedElements) {// 循环所有待注入的元素
			Member member = element.getMember();// 从待注入元素中获取属性或方法
			if (!beanDefinition.isExternallyManagedConfigMember(member)) {// 所有待注入的属性都需要注册到BeanDefinition的externallyManagedConfigMembers中
		...
		}// injectedElements只是所有找到的@Autowired的元素，只有被注册到beanDefinition的才会转化成checkedElements，checkedElements才是代表最终会被注入的元素
