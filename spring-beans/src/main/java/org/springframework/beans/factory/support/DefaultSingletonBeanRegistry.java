...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
		...
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
			synchronized (this.singletonObjects) {
				...
				if (singletonObject == null && allowEarlyReference) {
					 ...
					 * 但是并不是所有的bean都存在循环依赖，也就是三级缓存到二级缓存的步骤不一定都会被执行，有可能曝光后直接创建完成，
					...
					if (singletonFactory != null) {
						 ...
						 * 这也正是三级缓存存在的意义，用户可以通过重写该后置处理器对提前曝光的实例，在被提前引用时进行一些操作
 						 */
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				...
				// 循环依赖检测的典型做法，如果发现当前bean需要被循环依赖检测且正在创建则抛出异常，否则标识当前的bean正在创建
				...
				try {
					/**
					 * 回调singletonFactory的getObject方法，实际调用AbstractBeanFactory的createBean，而该方法又被子类所实现
	...
	public boolean isSingletonCurrentlyInCreation(String beanName) {
		return this.singletonsCurrentlyInCreation.contains(beanName);// 判断当前bean是否正在创建
	}
